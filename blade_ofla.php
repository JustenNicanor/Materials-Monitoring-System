...
// Fetch data for blade_ofla table
$table = 'blade_ofla';
$stmt = $conn->prepare("SELECT id, datetime, qty_in, qty_out, person_in_charge, total_bal_qty FROM blade_ofla ORDER BY datetime ASC");
if ($stmt === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update' && $id) {
        $stmt = $conn->prepare("UPDATE blade_ofla SET datetime=?, qty_in=?, qty_out=?, person_in_charge=?, total_bal_qty=? WHERE id=?");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
        ...
        $stmt = $conn->prepare("INSERT INTO blade_ofla (datetime, qty_in, qty_out, person_in_charge, total_bal_qty) VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
...
<div class="container">
    ...
    <form id="blade_oflaForm">
        <h2>blade_ofla Table</h2>
        <div class="table-container">
            <table id="blade_oflaTable">
                <thead>
                    <tr>
                        <th>Date & Time</th>
                        <th>Quantity In</th>
...
document.addEventListener('DOMContentLoaded', function() {
    function updateTotalBalQty() {
        const table = document.getElementById('blade_oflaTable');
