...
// Fetch data for capacitor_220uf table
$table = 'capacitor_220uf';
$stmt = $conn->prepare("SELECT id, datetime, qty_in, qty_out, person_in_charge, total_bal_qty FROM capacitor_220uf ORDER BY datetime ASC");
if ($stmt === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update' && $id) {
        $stmt = $conn->prepare("UPDATE capacitor_220uf SET datetime=?, qty_in=?, qty_out=?, person_in_charge=?, total_bal_qty=? WHERE id=?");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
        ...
        $stmt = $conn->prepare("INSERT INTO capacitor_220uf (datetime, qty_in, qty_out, person_in_charge, total_bal_qty) VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
...
<div class="container">
    ...
    <form id="capacitor_220ufForm">
        <h2>capacitor_220uf Table</h2>
        <div class="table-container">
            <table id="capacitor_220ufTable">
                <thead>
                    <tr>
                        <th>Date & Time</th>
                        <th>Quantity In</th>
...
document.addEventListener('DOMContentLoaded', function() {
    function updateTotalBalQty() {
        const table = document.getElementById('capacitor_220ufTable');
