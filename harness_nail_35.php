...
// Fetch data for harness_nail_35 table
$table = 'harness_nail_35';
$stmt = $conn->prepare("SELECT id, datetime, qty_in, qty_out, person_in_charge, total_bal_qty FROM harness_nail_35 ORDER BY datetime ASC");
if ($stmt === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update' && $id) {
        $stmt = $conn->prepare("UPDATE harness_nail_35 SET datetime=?, qty_in=?, qty_out=?, person_in_charge=?, total_bal_qty=? WHERE id=?");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
        ...
        $stmt = $conn->prepare("INSERT INTO harness_nail_35 (datetime, qty_in, qty_out, person_in_charge, total_bal_qty) VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
...
<div class="container">
    ...
    <form id="harness_nail_35Form">
        <h2>harness_nail_35 Table</h2>
        <div class="table-container">
            <table id="harness_nail_35Table">
                <thead>
                    <tr>
                        <th>Date & Time</th>
                        <th>Quantity In</th>
...
document.addEventListener('DOMContentLoaded', function() {
    function updateTotalBalQty() {
        const table = document.getElementById('harness_nail_35Table');
