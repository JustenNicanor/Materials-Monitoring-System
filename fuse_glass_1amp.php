...
// Fetch data for fuse_glass_1amp table
$table = 'fuse_glass_1amp';
$stmt = $conn->prepare("SELECT id, datetime, qty_in, qty_out, person_in_charge, total_bal_qty FROM fuse_glass_1amp ORDER BY datetime ASC");
if ($stmt === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update' && $id) {
        $stmt = $conn->prepare("UPDATE fuse_glass_1amp SET datetime=?, qty_in=?, qty_out=?, person_in_charge=?, total_bal_qty=? WHERE id=?");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
        ...
        $stmt = $conn->prepare("INSERT INTO fuse_glass_1amp (datetime, qty_in, qty_out, person_in_charge, total_bal_qty) VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
...
<div class="container">
    ...
    <form id="fuse_glass_1ampForm">
        <h2>fuse_glass_1amp Table</h2>
        <div class="table-container">
            <table id="fuse_glass_1ampTable">
                <thead>
                    <tr>
                        <th>Date & Time</th>
                        <th>Quantity In</th>
...
document.addEventListener('DOMContentLoaded', function() {
    function updateTotalBalQty() {
        const table = document.getElementById('fuse_glass_1ampTable');
