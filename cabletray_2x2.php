...
// Fetch data for cabletray_2x2 table
$table = 'cabletray_2x2';
$stmt = $conn->prepare("SELECT id, datetime, qty_in, qty_out, person_in_charge, total_bal_qty FROM cabletray_2x2 ORDER BY datetime ASC");
if ($stmt === false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update' && $id) {
        $stmt = $conn->prepare("UPDATE cabletray_2x2 SET datetime=?, qty_in=?, qty_out=?, person_in_charge=?, total_bal_qty=? WHERE id=?");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
        ...
        $stmt = $conn->prepare("INSERT INTO cabletray_2x2 (datetime, qty_in, qty_out, person_in_charge, total_bal_qty) VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . $conn->error);
        }
...
<div class="container">
    ...
    <form id="cabletray_2x2Form">
        <h2>cabletray_2x2 Table</h2>
        <div class="table-container">
            <table id="cabletray_2x2Table">
                <thead>
                    <tr>
                        <th>Date & Time</th>
                        <th>Quantity In</th>
...
document.addEventListener('DOMContentLoaded', function() {
    function updateTotalBalQty() {
        const table = document.getElementById('cabletray_2x2Table');
